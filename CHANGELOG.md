...
### Changed
- **A bare data question answers the latest value, not the last 7 days.**
  `novrix "btc price and fg"` now answers each indicator with its current
  value on one line (`BTC PRICE · $62.76K · 2026-08-03`), exactly like
  the plain `novrix btc` command. A history table only appears when you
  ask for one — `last 10 days of fear and greed`, a date range, or
  `--days N`.
- **Novrix-stance answers stay on the market.** `is novrix bullish now?`
  no longer drags Dero / privacy coins into the answer (and no longer
  gets cut off mid-sentence on them) — that topic belongs to the separate
  @techleaks24 privacy analyzer. The answer covers novrix's stance, what
  it posts and forwards, Bull Case's thesis and its AI take, then stops.
